...
if (isset($_POST['update'])) {
    ...
    $result = mysqli_query($conn, "SELECT password FROM alogin WHERE id = $id");
    $employee = mysqli_fetch_assoc($result);
    
    if ($old == $employee['password']) {
        $sql = "UPDATE `alogin` SET `password`='$new' WHERE id = $id";
        mysqli_query($conn, $sql);
        echo ("<SCRIPT LANGUAGE='JavaScript'>
            window.alert('Đổi mật khẩu thành công')
            window.location.href='aloginwel.php';</SCRIPT>");
    } else {
        echo ("<SCRIPT LANGUAGE='JavaScript'>
...
<body>
    ...
    <div class="container mt-5">
        <div class="card">
            ...
            <div class="card-body">
                <form action="adchangepwd.php" method="POST">
